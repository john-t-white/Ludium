...
the one command an agent posts a round with. The name prefix, the severity, the
anchor, and the link that pairs two threads on one problem are added by that
command, because every one of them was a rule in prose that an agent broke at
least once. This document says why the review works the way it does; those
files and that command are what actually runs, so a change to how a review runs
is a change to them. Each agent runs at a capability level matched to its job,
recorded in its frontmatter. What a review flags, at what severity, how much of
it, and how a finding is filed is calibration rather than process, and it lives
in [REVIEW.md](REVIEW.md), which a review reads and nothing else does. The three files divide cleanly: `CLAUDE.md` is
